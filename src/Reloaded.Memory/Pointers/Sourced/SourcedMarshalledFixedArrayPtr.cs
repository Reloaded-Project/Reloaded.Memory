...
public unsafe struct SourcedMarshalledFixedArrayPtr<
#if NET5_0_OR_GREATER
    ...
    T, TSource> : IEnumerable<T> where T : unmanaged where TSource : ICanReadWriteMemory
{
    ...
    public SourcedMarshalledFixedArrayPtr(T* address, int count, TSource source)
    {
        Pointer = new MarshalledFixedArrayPtr<T>((byte*)address, count);
        Source = source;
    }
