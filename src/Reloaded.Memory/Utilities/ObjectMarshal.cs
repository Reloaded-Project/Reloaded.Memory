...
public static class ObjectMarshal
{
    ...
    ///     for the parameter <paramref name="obj" />. The offset is in relation to the first usable byte after the method
    ///     table.
    /// </returns>
    /// <remarks>
    ...
    /// <param name="value">
    ///     The resulting <typeparamref name="T" /> value, if <paramref name="obj" /> was in fact a boxed
    ///     <typeparamref name="T" /> value.
    /// </param>
    /// <returns>
    ///     <see langword="true" /> if a <typeparamref name="T" /> value was retrieved correctly, <see langword="false" />
    ///     otherwise.
    /// </returns>
    /// <remarks>
    ...
    public static ref T DangerousUnbox<T>(object obj)
        where T : struct => ref Unsafe.Unbox<T>(obj);
}
